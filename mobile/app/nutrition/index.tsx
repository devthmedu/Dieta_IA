...
export default function Nutrition() {
  ...
  const { data, isFetching, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        ...
        throw new Error("Erro ao gerar nutrição"); // Lança o erro para que o React Query possa capturá-lo
      }
    },
  });
  ...
  const handleShare = async () => {
    try {
      ...
      const supplements = `${data?.supplements.join(", ")}`; // Usando join para formatar melhor
      const dietPlan = data?.diet_plan.map(
        (item) =>
          `\n- Nome: ${item.name}\n- Horário: ${item.time}\n- Alimentos: ${item.meals.join(", ")}`
      ).join(""); // Usando join para formatar melhor
